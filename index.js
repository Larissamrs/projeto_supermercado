...
console.log ('Realizado por Larissa Maria')
console.log ('----------------------------------------------------------')
...
if (categoria.toUpperCase() === 'S'){
    console.log('As categorias listadas:');
    console.log('alimento', 'informatica', 'casa', 'higiene', 'bebida');

    const categoria = read.question('Qual categoria voce escolhe?');

    const filtrarCategoria = produtos.filter(produtos => produtos.categoria === categoria);
...
const compras = () =>{
    ...
    for (i=0; i<1000; i++){
        ...
        const produtosDoCarrinho = {...findId, quantidade: quantidadeItens}
...
const dateToday = hoje.toLocaleDateString('pt-BR', options)
